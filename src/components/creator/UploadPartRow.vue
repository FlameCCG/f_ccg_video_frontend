...
import { Clapperboard, Layers3, Pause, RefreshCw, RotateCcw, X } from 'lucide-vue-next'
...
const statusLabel = computed(() => {
  if (isInstant.value) return '秒传成功'
...
const detail = computed(() => {
  ...
  switch (part.status) {
    ...
    case 'success':
      return formatBytes(part.file.size)
    default:
      return ''
  }
})
...
<template>
  ...
  >
    <div
      class="upload-part__badge"
      aria-hidden="true"
      v-bind="dataAttrs({ 'data-multipart': showLabel ? 'true' : 'false' })"
    >
      <Layers3 v-if="showLabel" class="upload-part__badge-icon" />
      <Clapperboard v-else class="upload-part__badge-icon" />
      <span v-if="showLabel" class="upload-part__badge-index tabular">P{{ index + 1 }}</span>
    </div>
...
.upload-part__badge {
  ...
  height: 2.25rem;
  overflow: hidden;
  border: 1px solid color-mix(in oklch, var(--tone-ink) 20%, var(--color-border));
  border-radius: 0.375rem 0.75rem;
  background: linear-gradient(
    135deg,
    color-mix(in oklch, var(--tone-soft) 88%, var(--color-card)),
    var(--tone-soft)
  );
  color: var(--tone-ink);
  transition:
...
.upload-part__badge-icon {
  width: 1rem;
  height: 1rem;
}

.upload-part__badge-index {
  position: absolute;
  right: 0.25rem;
  bottom: 0.125rem;
  font-size: 0.625rem;
