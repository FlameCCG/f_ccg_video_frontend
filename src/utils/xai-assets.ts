import { resolveXaiAssetUrl } from '@/api/xai'
...
export const buildXaiSuggestedTitle = (prompt: string, fallback: string) => {
  return sanitizeBaseName(prompt, fallback)
}

export const fetchXaiAssetAsFile = async (
  assetUrl: string,
  baseName: string,
  fallbackExtension: string
) => {
  const resolvedUrl = resolveXaiAssetUrl(assetUrl)
  ...
  const fileName = `${sanitizeBaseName(baseName, 'xai-asset')}.${extension}`
