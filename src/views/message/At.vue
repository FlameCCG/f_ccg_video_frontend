  ...
  <div class="flex h-full flex-col">
    ...
    >
      ...
      <EmptyState
        ...
        icon="at"
        title="还没有人 @ 你"
        description="别人在评论或弹幕里提到你时，消息会出现在这里"
      />
      ...
      <div v-else class="space-y-4">
        ...
        >
          ...
          <div class="flex min-w-0 flex-1 flex-col justify-center">
            ...
            <div v-if="item.content" class="mb-2">
              <button
                ...
                type="button"
                class="-ml-2 inline-flex max-w-full cursor-pointer rounded-md px-2 py-1 text-left text-sm text-foreground/90 t-tint hover:bg-primary/8 hover:text-primary"
                @click="goLink(item)"
              >
          ...
          <div class="flex shrink-0 items-start gap-2">
            ...
            <div v-if="item.videoTitle" class="max-w-[120px] cursor-pointer" @click="goLink(item)">
              <div
                class="h-[60px] flex items-center justify-center overflow-hidden rounded bg-muted text-xs text-muted-foreground p-2 line-clamp-2 hover:text-primary t-tint"
              >
                {{ item.videoTitle }}
              </div>
            </div>
