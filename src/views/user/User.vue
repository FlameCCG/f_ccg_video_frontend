      ...
      <div class="h-header relative h-[220px]">
            ...
            <Navbar class="text-white">
              <template #after-actions="{ actionTextClass }">
                <button v-if="isSelf" type="button"
                  ...
                  ]" :aria-expanded="bannerPickerOpen" :title="bannerPickerOpen ? '收起横幅面板' : '打开横幅面板'"
                  aria-label="切换横幅面板" @click="toggleBannerPicker">
                  <Shirt class="h-4 w-4" />
                </button>
              </template>
            </Navbar>
        ...
        <div
          ...
          <div class="w-full px-4 md:px-8 xl:px-[140px] flex items-end">
            <AppAvatar :src="user.avatar" :name="user.username"
            ...
            <div class="ml-5 text-white mb-1">
              <div class="flex items-center gap-2">
                ...
                <span class="px-1.5 py-0.5 rounded text-[12px] font-bold shadow-sm"
            ...
            <div class="ml-auto mb-2 flex items-center gap-3">
              ...
              <template v-else>
                <button class="px-8 py-1.5 rounded text-white text-[14px] font-medium transition-colors" :class="isFollowed
                ...
                <button
                  ...
                  :disabled="chatOpening" @click="handleOpenChat">
        ...
        <div class="w-full px-4 md:px-8 xl:px-[140px] flex items-center h-[50px]">
          <div class="flex gap-6 h-full">
            <button v-for="t in tabs" :key="t.key"
              ...
              :class="activeTab === t.key ? 'text-primary font-medium' : 'text-foreground'" @click="switchTab(t.key)">
              <component :is="t.icon" class="mr-1" :size="16" />
              {{ t.label }}
              <div v-if="activeTab === t.key"
                class="absolute bottom-0 left-1/2 -translate-x-1/2 w-5 h-[3px] bg-primary rounded-t-sm"></div>
            </button>
          </div>

          <div class="ml-auto flex items-center gap-5 text-[13px] text-muted-foreground">
            <div class="flex flex-col items-center leading-tight cursor-pointer transition-colors"
              @click="switchTab('following')">
              <span class="font-medium text-[14px] transition-colors"
                :class="activeTab === 'following' ? 'text-primary' : 'text-foreground'">{{ fmtCount(user.followCount)
                }}</span>
              <span class="text-[11px]" :class="activeTab === 'following' ? 'text-primary' : ''">关注数</span>
            </div>
            <div class="flex flex-col items-center leading-tight cursor-pointer transition-colors"
              @click="switchTab('fans')">
              <span class="font-medium text-[14px] transition-colors"
                :class="activeTab === 'fans' ? 'text-primary' : 'text-foreground'">{{ fmtCount(user.fansCount) }}</span>
              <span class="text-[11px]" :class="activeTab === 'fans' ? 'text-primary' : ''">粉丝数</span>
            </div>
      ...
      <div class="w-full px-4 md:px-8 xl:px-[140px] pt-5 pb-12">
          ...
          <div class="flex flex-col gap-5 xl:flex-row xl:items-start">
            <div class="min-w-0 flex-1">
              ...
              <div>
                <div class="mb-4 flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="flex gap-1">
                      <button v-for="o in sortOpts" :key="o.value"
                        class="px-3 py-0.5 rounded-full text-[12px] transition-colors" :class="homeVideoSort === o.value
                ...
                <div v-if="homeVideos.length > 0" class="grid grid-cols-4 gap-4">
                  <div v-for="v in homeVideos" :key="v.id" class="u-video-card" @click="goVideo(v.id)">
                    <div class="uvc-cover">
                      <img :src="v.cover" />
                      <div class="uvc-stats">
                        <span>
                          <Play :size="11" /> {{ fmtCount(v.views) }}
                        </span>
                        <span>
                          <MessageSquare :size="11" /> {{ fmtCount(v.danmuCount) }}
                        </span>
                      </div>
                ...
                <div v-else-if="!homeVideosLoading" class="py-10 text-center text-[13px] text-muted-foreground/80">
                  暂无投稿视频
                </div>
              </div>
            </div>
            ...
            <div class="w-full flex-shrink-0 space-y-4 xl:w-[260px]">
              ...
              <div class="rounded-lg bg-secondary p-4">
                <div class="mb-2 border-b border-border pb-2 text-[14px] font-medium text-foreground">
                  个人资料
                </div>
                  ...
                  <template v-for="item in profileMetaItems" :key="item.key">
                    <div v-if="'value' in item" class="flex items-center gap-2 text-muted-foreground">
                      ...
                      <i v-else :class="item.icon"
                        class="text-[18px] w-[18px] h-[18px] not-italic inline-flex items-center justify-center flex-shrink-0 text-muted-foreground/75"></i>
                    ...
                    <div v-else class="grid grid-cols-2 gap-x-2 gap-y-2.5 mt-2">
                      <div v-for="tag in item.tags" :key="`${item.key}-${tag}`"
                        class="flex items-center gap-1.5 text-foreground/90 overflow-hidden">
                        <i
                          class="vui_icon sic-fsp-tag_line text-[14px] not-italic inline-flex items-center justify-center text-muted-foreground/80 shrink-0"></i>
          ...
          <div class="space-y-8 xl:pr-[280px]">
            ...
            <div v-if="likedVideos.length > 0">
              ...
              <div class="grid grid-cols-4 gap-4">
                <div v-for="v in likedVideos.slice(0, 8)" :key="v.id" class="u-video-card" @click="goVideo(v.id)">
                  <div class="uvc-cover">
                    <img :src="v.cover" />
                    <div class="uvc-stats">
                      <span>
            ...
            <div v-if="coinedVideos.length > 0">
              ...
              <div class="grid grid-cols-4 gap-4">
                <div v-for="v in coinedVideos.slice(0, 8)" :key="v.id" class="u-video-card" @click="goVideo(v.id)">
                  <div class="uvc-cover">
                    <img :src="v.cover" />
                    <div class="uvc-stats">
                      <span>
            ...
            <div v-if="folders.length > 0">
              <div class="mb-4 flex items-center justify-between">
                ...
                <button class="flex items-center text-[12px] text-muted-foreground transition-colors hover:text-primary"
              ...
              <div class="grid grid-cols-4 gap-4">
                <div v-for="f in folders.slice(0, 4)" :key="f.id" class="u-folder-card"
                  @click="router.push({ path: '/favorites', query: { folderId: String(f.id) } })">
                  <div class="ufc-cover">
                    <img v-if="folderCovers[f.id]" :src="folderCovers[f.id]" class="ufc-cover-img" />
        ...
        <div v-else-if="activeTab === 'dynamic'" class="flex gap-5 items-start">
          ...
          <div class="w-[180px] flex-shrink-0 bg-secondary rounded-lg py-3">
            <button class="w-full text-left px-5 py-2 text-[13px] transition-colors" :class="dynamicFilter === 'all'
              ? 'bg-primary text-white'
              : 'text-muted-foreground hover:bg-secondary'
              " @click="switchDynamicFilter('all')">
              全部
            </button>
            <button class="w-full text-left px-5 py-2 text-[13px] transition-colors" :class="dynamicFilter === 'video'
              ? 'bg-primary text-white'
              : 'text-muted-foreground hover:bg-secondary'
              " @click="switchDynamicFilter('video')">
              视频
            </button>
            <button class="w-full text-left px-5 py-2 text-[13px] transition-colors" :class="dynamicFilter === 'image'
              ? 'bg-primary text-white'
              : 'text-muted-foreground hover:bg-secondary'
          ...
          <div class="flex-1 min-w-0 space-y-4">
            ...
            <div v-if="isSelf" class="border border-border rounded-lg p-5">
              <div class="flex gap-3">
                <AppAvatar :src="user.avatar" :name="user.username" container-class="h-10 w-10 flex-shrink-0"
                  text-class="text-sm font-semibold" />
                <div class="flex-1">
                  <textarea v-model="newDynamicContent"
                    ...
                    rows="3" placeholder="有什么想和大家分享的？"></textarea>
                  ...
                  <div class="flex items-center justify-between mt-2">
                    <label class="cursor-pointer text-muted-foreground/80 hover:text-primary transition-colors">
                      <ImagePlus :size="18" />
                      <input type="file" accept="image/*" class="hidden" @change="handleDynamicImageUpload" />
                    </label>
                    <button
                      ...
                      :disabled="dynamicPublishing || !newDynamicContent.trim()" @click="publishDynamic">
            ...
            <!-- Dynamic Feed -->
            <div v-for="item in filteredDynamics" :key="`${item.workType}-${getWorkId(item)}`"
              class="border border-border rounded-lg p-5">
              <div class="flex gap-3">
                <div class="flex-shrink-0 cursor-pointer" @click="router.push(`/user/${item.author.id}`)">
                  <AppAvatar :src="item.author.avatar" :name="item.author.username" container-class="h-10 w-10"
                    text-class="text-sm font-semibold" />
                </div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center justify-between">
                    <div>
                      <span class="text-[14px] font-medium text-primary cursor-pointer hover:text-primary/80"
                        @click="router.push(`/user/${item.author.id}`)">{{ item.author.username }}</span>
                      <span class="text-[12px] text-muted-foreground/80 ml-2">{{
                        fmtFullTime(item.createdAt)
                      }}</span>
                      <span v-if="item.workType === 1" class="text-[11px] text-muted-foreground/80 ml-2">· 投稿了视频</span>
                    </div>
                    <DropdownMenu v-if="isSelf">
                      <DropdownMenuTrigger as-child>
                        <button class="p-1 text-muted-foreground/80 hover:text-muted-foreground transition-colors">
                          <MoreVertical :size="16" />
                        </button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem class="cursor-pointer text-[13px]" @select="handlePinDynamic(item)">
                          <Pin class="mr-2 h-4 w-4" />
                          {{ item.dynamic?.isPinned || item.video?.isPinned ? '取消置顶' : '置顶' }}
                        </DropdownMenuItem>
                        <DropdownMenuItem v-if="item.workType === 2" class="cursor-pointer text-[13px] text-red-500"
                  ...
                  <!-- Pinned badge -->
                  <div v-if="item.dynamic?.isPinned || item.video?.isPinned"
                  ...
                  <div v-if="item.workType === 2 && item.dynamic" class="mt-2">
                    ...
                    <img v-if="item.dynamic.imageUrl" :src="item.dynamic.imageUrl"
                      class="mt-3 max-w-[360px] rounded-lg cursor-pointer" />
                  </div>

                  <!-- Video card in dynamic -->
                  <div v-if="item.workType === 1 && item.video"
                    ...
                    <div class="relative w-[160px] aspect-video rounded overflow-hidden flex-shrink-0 bg-accent">
                  ...
                  <div class="mt-4 flex flex-wrap items-center gap-2 border-t border-border/60 pt-3">
                    <button
                      class="inline-flex min-w-[78px] items-center gap-1.5 rounded-full px-3 py-1.5 text-[13px] transition-colors"
                      :class="isCommentExpanded(item)
                  ...
                  <div v-if="isCommentExpanded(item)" class="mt-4 border-t border-border/50 pt-4">
                    <CommentSection :video-id="getVideoId(item)" :dynamic-id="getDynamicId(item)"
            ...
            <div v-if="filteredDynamics.length === 0 && !dynamicsLoading"
            ...
            <div v-if="dynamics.length < dynamicTotal" class="text-center pt-2">
              <button
                ...
                :disabled="dynamicsLoading" @click="loadMoreDynamics">
        ...
        <!-- ==================== VIDEOS TAB ==================== -->
        <div v-else-if="activeTab === 'videos'" class="flex gap-4 items-start bg-card rounded-lg min-h-[500px]">
          <div class="w-[160px] flex-shrink-0 border-r border-border py-3">
            ...
            <div class="flex items-center justify-between px-4 py-2 bg-primary text-white cursor-pointer">
          ...
          <div class="flex-1 p-5">
            <div class="flex items-center justify-between mb-5">
              ...
              <div class="flex bg-secondary rounded-full p-0.5">
                <button v-for="o in sortOpts" :key="o.value"
                  class="px-3 py-1 rounded-full text-[12px] transition-colors" :class="videoSort === o.value
            ...
            <div v-if="videos.length > 0" class="grid grid-cols-6 gap-5">
              <div v-for="v in videos" :key="v.id" class="u-video-card cursor-pointer hover-lift"
                @click="goVideo(v.id)">
                <div class="uvc-cover">
                  <img :src="v.cover" />
                  <div class="uvc-stats">
            ...
            <div v-else-if="!videosLoading" class="py-16 text-center text-muted-foreground/80 text-[13px]">
              暂无视频
            </div>
            ...
            <div v-if="videos.length < videoTotal" class="text-center mt-6">
              <button
                class="px-6 py-1.5 rounded border border-border text-muted-foreground text-[12px] hover:text-primary hover:border-primary transition-colors"
                :disabled="videosLoading" @click="loadMoreVideos">
        ...
        <!-- ==================== FAVORITES TAB ==================== -->
        <div v-else-if="activeTab === 'favorites'" class="flex gap-4 items-start bg-card rounded-lg min-h-[500px]">
          <div class="w-[180px] flex-shrink-0 border-r border-border py-3">
            ...
            <div v-for="f in folders" :key="f.id"
              class="flex items-center justify-between px-4 py-2.5 cursor-pointer transition-colors" :class="selectedFolderId === f.id
                ? 'bg-primary text-white'
                : 'text-muted-foreground hover:bg-secondary'
                " @click="selectFolder(f.id)">
              <span class="text-[13px] truncate flex-1">{{ f.name }}</span>
              <span class="text-[11px] ml-2" :class="selectedFolderId === f.id
          ...
          <div v-if="selectedFolder" class="flex-1 p-5">
            <div class="flex items-center justify-between mb-5 pb-5 border-b border-border">
              ...
              <div class="flex items-center gap-3">
                <div class="flex bg-secondary rounded-full p-0.5">
                  <button v-for="o in folderSortOpts" :key="o.value"
                    class="px-3 py-1 rounded-full text-[12px] transition-colors" :class="folderVideoSort === o.value
                      ? 'bg-card text-primary shadow-sm'
                      : 'text-muted-foreground hover:text-foreground'
            ...
            <div v-if="folderVideos.length > 0" class="grid grid-cols-6 gap-5">
              <div v-for="v in folderVideos" :key="v.id" class="u-video-card" @click="goVideo(v.id)">
                <div class="uvc-cover">
                  <img :src="v.cover" />
                  <div class="uvc-stats">
                    <span>
            ...
            <div v-else-if="!folderVideosLoading" class="py-16 text-center text-muted-foreground/80 text-[13px]">
              收藏夹为空
            </div>
            ...
            <div v-if="folderVideos.length < folderVideoTotal" class="text-center mt-6">
              <button
                class="px-6 py-1.5 rounded border border-border text-muted-foreground text-[12px] hover:text-primary hover:border-primary transition-colors"
                :disabled="folderVideosLoading" @click="loadMoreFolderVideos">
          ...
          <div v-else-if="userConf" class="space-y-8">
            <div>
              ...
              <div class="grid grid-cols-3 gap-x-8 gap-y-5">
                <div v-for="s in privacySettings" :key="s.key" class="flex items-center justify-between">
                  <span class="text-[13px] text-foreground">{{ s.label }}</span>
                  <button class="relative h-[22px] w-[40px] cursor-pointer rounded-full transition-colors" :class="(userConf as Record<string, unknown>)[s.key]
                    ? 'bg-primary'
                    : 'bg-black/10 dark:bg-white/15'
                    " :disabled="confSaving" @click="toggleConf(s.key)">
                    <span
                      class="absolute left-0 top-[2px] h-[18px] w-[18px] rounded-full bg-white shadow transition-transform"
                      :class="(userConf as Record<string, unknown>)[s.key]
                        ? 'translate-x-[20px]'
                        : 'translate-x-[2px]'
                        "></span>
                  </button>
            ...
            <div class="mt-8">
              ...
              <div class="flex flex-wrap items-center gap-3">
                <template v-if="userConf.likeTags.length">
                  <span v-for="tag in userConf.likeTags" :key="tag"
                    ...
                    <i class="vui_icon sic-fsp-tag_line mt-[1px] text-[14px] text-muted-foreground not-italic"></i>
                    {{ tag }}
                    <button type="button"
                      ...
                      aria-label="删除标签" @click="removeLikeTagDraft(tag)">
                ...
                <div class="flex items-center h-[34px] shadow-sm rounded">
                  <div class="relative w-[200px] h-full">
                    <input v-model="likeTagInput" type="text" maxlength="24"
        ...
        <!-- ==================== FOLLOWING / FANS TAB ==================== -->
        <div v-else-if="activeTab === 'following' || activeTab === 'fans'"
          ...
          <div class="w-[180px] flex-shrink-0 border-r border-border py-3">
            ...
            <div class="flex items-center justify-between px-4 py-2.5 cursor-pointer transition-colors" :class="activeTab === 'following'
              ? 'bg-primary text-white'
              : 'text-muted-foreground hover:bg-secondary'
              " @click="switchTab('following')">
              <span class="text-[13px]">全部关注</span>
              <span class="text-[11px] ml-2" :class="activeTab === 'following'
                ? 'bg-card/20 px-1.5 rounded'
                : 'text-muted-foreground/80'
            ...
            <div class="flex items-center justify-between px-4 py-2.5 cursor-pointer transition-colors" :class="activeTab === 'fans'
              ? 'bg-primary text-white'
              : 'text-muted-foreground hover:bg-secondary'
              " @click="switchTab('fans')">
              <span class="text-[13px]">{{ isSelf ? '我的粉丝' : 'TA的粉丝' }}</span>
              <span class="text-[11px] ml-2" :class="activeTab === 'fans' ? 'bg-card/20 px-1.5 rounded' : 'text-muted-foreground/80'
          ...
          <div class="flex-1 p-5">
            <div class="flex items-center justify-between mb-5">
              ...
              <div v-if="activeTab === 'following'" class="relative">
                <input v-model="followingKeyword"
                  ...
                  placeholder="搜索关注" @keyup.enter="searchFollowing" />
                <Search
                  ...
                  :size="16" @click="searchFollowing" />
              </div>
              <div v-else class="relative">
                <input v-model="fansKeyword"
                  class="w-[200px] h-[32px] pl-3 pr-8 rounded border border-border text-[13px] focus:outline-none focus:border-primary transition-colors"
                  placeholder="搜索粉丝" @keyup.enter="searchFans" />
                <Search
                  class="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground/80 cursor-pointer hover:text-primary transition-colors"
                  :size="16" @click="searchFans" />
              </div>
            </div>
            ...
            <template v-if="activeTab === 'following'">
              <div v-if="followingList.length > 0" class="grid grid-cols-3 gap-x-4 gap-y-1">
                <div v-for="u in followingList" :key="u.id" class="sc-card">
                  <div @click="router.push(`/user/${u.id}`)">
                    <AppAvatar :src="u.avatar" :name="u.username" :alt="u.username" container-class="sc-avatar"
                  ...
                  <button v-if="isSelf && u.id !== authStore.userId" class="sc-btn"
              ...
              <div v-if="followingList.length === 0 && !followingLoading"
                class="py-16 text-center text-muted-foreground/80 text-[13px]">
                {{ followingKeyword ? '未找到匹配的关注用户' : '暂无关注' }}
              </div>
              <div v-if="followingLoading && followingList.length === 0"
                class="py-16 text-center text-muted-foreground/80 text-[13px]">
                加载中...
              </div>
              <div v-if="followingList.length > 0 && followingList.length < followingTotal" class="text-center mt-6">
                <button
                  class="px-6 py-1.5 rounded border border-border text-muted-foreground text-[12px] hover:text-primary hover:border-primary transition-colors"
                  :disabled="followingLoading" @click="loadMoreFollowing">
            ...
            <template v-else>
              <div v-if="fansList.length > 0" class="grid grid-cols-3 gap-x-4 gap-y-1">
                <div v-for="u in fansList" :key="u.id" class="sc-card">
                  ...
                  <button v-if="authStore.isLoggedIn && u.id !== authStore.userId" class="sc-btn"
                    :class="isUserFollowedByMe(u.id) ? 'sc-btn-followed' : 'sc-btn-follow'"
                    @click.stop="handleFollowInList(u.id)">
                    <UserCheck v-if="isUserFollowedByMe(u.id)" :size="12" class="mr-1" />
                    {{ isUserFollowedByMe(u.id) ? '已关注' : '关注' }}
              ...
              <div v-if="fansLoading && fansList.length === 0"
                class="py-16 text-center text-muted-foreground/80 text-[13px]">
                加载中...
              </div>
              <div v-if="fansList.length > 0 && fansList.length < fansTotal" class="text-center mt-6">
                <button
                  class="px-6 py-1.5 rounded border border-border text-muted-foreground text-[12px] hover:text-primary hover:border-primary transition-colors"
                  :disabled="fansLoading" @click="loadMoreFans">
        ...
        <Teleport to="body">
          <transition name="banner-sheet">
            <div v-if="isSelf && bannerPickerOpen" class="fixed inset-x-0 bottom-0 z-[70] pointer-events-none">
              <div
                ...
                <UserBannerPicker :open="bannerPickerOpen" class="!rounded-none !border-x-0 !border-b-0 !shadow-none"
                  @close="closeBannerPicker" @preview="handleBannerPreview" @saved="handleBannerSaved" />
              </div>
            </div>
          </transition>
        </Teleport>
