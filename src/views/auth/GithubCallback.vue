...
const handleGithubCallback = async () => {
  ...
  if (oauthError) {
    clearOAuthSession('github')
    errorMessage.value = oauthErrorDescription || 'GitHub 授权已取消或失败，请重新登录'
  ...
  const session = consumeOAuthSession('github')
  if (!session?.codeVerifier || !session.state) {
    errorMessage.value = 'GitHub 登录状态已失效，请重新发起登录'
    isLoading.value = false
    return
  }

  if (!state || state !== session.state) {
    errorMessage.value = 'GitHub 登录状态校验失败，请重新发起登录'
    isLoading.value = false
    return
  }
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'GitHub 登录失败，请重试'
  } finally {
    isLoading.value = false
  }
}
  ...
  >
    ...
    <div class="relative flex flex-col items-center gap-6 text-center">
      <template v-if="isLoading">
        ...
        <div class="space-y-2">
          <h1 class="text-2xl font-bold">GitHub 登录中</h1>
